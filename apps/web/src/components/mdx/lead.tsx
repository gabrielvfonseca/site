 ...
 */
export function Lead({ children, dropcap = false, className }: LeadProps) {
	return (
		<div
			className={cn(
				"text-balance text-foreground text-lg leading-relaxed sm:text-xl [&>p]:m-0",
				dropcap &&
					"[&>p::first-letter]:float-left [&>p::first-letter]:mr-2 [&>p::first-letter]:text-5xl [&>p::first-letter]:leading-[0.8]",
				className,
			)}
		>
			{children}
