...
export function Bookmark({
	...
	return (
		<Link
			className={cn(
				"not-prose group flex items-stretch justify-between gap-4 overflow-hidden rounded-lg border border-border bg-muted/50 no-underline transition-colors duration-200 hover:border-foreground/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 motion-reduce:transition-none",
				className,
			)}
			href={href}
		...
		>
			...
			{image ? (
				<span className="hidden w-40 shrink-0 overflow-hidden border-border border-l sm:block">
					...
					{/* biome-ignore lint/correctness/useImageSize: intrinsic size is unknown for authored MDX */}
					<img
						alt=""
