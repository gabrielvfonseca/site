...
function PinnedBadge() {
	return (
		<span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-micro text-muted-foreground uppercase tracking-wide">
			<svg
				aria-hidden
				className="size-2.5"
				fill="currentColor"
...
function AmaItem({ item }: { item: PublicAmaEntry }) {
	const content = (
		<>
	...
	return (
		<li>
			<Link
				...
				className="-mx-3 flex flex-col gap-1.5 rounded-lg px-3 py-2 transition-colors duration-200 hover:bg-muted/50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
				href={`/ama/${item.slug}`}
			>
				{content}
				<span className="mt-0.5 text-muted-foreground/[var(--opacity-secondary)] text-xs">
					Read thread →
				</span>
			</Link>
		</li>
 ...
 */
export function AmaList({ items }: AmaListProps) {
	if (items.length === 0) {
		return (
