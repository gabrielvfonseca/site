		...
		<ol className="flex flex-col">
			{tracks.map((track, index) => (
				<li key={`${track.name}-${track.artist}`}>
					<Link
						className="group -mx-2 flex items-center gap-3 rounded-lg px-2 py-2 transition-colors duration-200 hover:bg-muted/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
						href={track.url}
					...
					>
						...
						{track.albumImage ? (
							...
							// biome-ignore lint/correctness/useImageSize: intrinsic size is unknown
							<img
								alt=""
						...
						<span className="flex min-w-0 flex-col">
							<span className="truncate text-foreground text-sm leading-5">
								{track.name}
							</span>
