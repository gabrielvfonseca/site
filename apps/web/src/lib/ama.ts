...
import type { AmaFormData } from "@/types/ama";
...
/**
 * Whether a database is configured for this environment.
 *
 * Running without Supabase credentials is a supported state — the AMA page
 * degrades to an empty list — so it must not be reported as an application
 * error. Only genuine query failures reach {@link parseError}.
 * @returns `true` when Supabase credentials are present.
 */
async function isDatabaseConfigured(): Promise<boolean> {
	try {
		const { isSupabaseConfigured } = await import("@gabfon/supabase/keys");
...
export async function getPublishedQuestions(): Promise<PublicAmaEntry[]> {
	...
	try {
		const { createClient } = await import("@gabfon/supabase/client");
		const { getPublishedAmaQuestionsQuery } = await import(
			"@gabfon/supabase/queries"
		);
		const client = createClient();
		const rows = await getPublishedAmaQuestionsQuery(client);
		return rows.map((row) => ({
			id: row.id,
			slug: row.slug,
...
export async function getThreadBySlug(slug: string): Promise<AmaThread | null> {
	...
	try {
		const { createClient } = await import("@gabfon/supabase/server");
		const { getAmaQuestionBySlugQuery, getAmaMessagesByQuestionIdQuery } =
			await import("@gabfon/supabase/queries");
		const client = await createClient();
		const head = await getAmaQuestionBySlugQuery(client, slug);
		if (!head) {
			return null;
		}
		const messages = await getAmaMessagesByQuestionIdQuery(client, head.id);
		return {
			id: head.id,
			slug: head.slug,
			...
			answer: head.answer ?? "",
			pinned: head.pinned,
			askerName: head.asker_name,
			answeredAt: head.answered_at,
...
export async function submitQuestion(data: AmaFormData): Promise<boolean> {
	...
	if (await isDatabaseConfigured()) {
		try {
			const { createClient } = await import("@gabfon/supabase/client");
			const { insertAmaQuestion } = await import("@gabfon/supabase/mutations");
			const client = createClient();
			await insertAmaQuestion(client, {
				question,
