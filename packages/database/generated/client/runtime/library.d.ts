...
export declare type Action =
  | keyof typeof dmmf2.ModelAction
  | 'executeRaw'
  | 'queryRaw'
  | 'runCommandRaw';
...
export declare type Args3<T, F extends Operation> = Args<T, F>;
...
export declare type BaseDmmf = {
  readonly datamodel: Omit<dmmf2.Datamodel, 'indexes'>;
};
...
declare type BatchTransactionOptions = {
  isolationLevel?: transaction2.IsolationLevel;
};
...
declare type Context2<T> = T extends {
  [k: symbol]: {
    ctx: infer C;
  };
}
...
declare type Datamodel = ReadonlyDeep2<{
  models: Model[];
  enums: DatamodelEnum[];
  types: Model[];
  indexes: Index[];
...
declare type DatamodelEnum = ReadonlyDeep2<{
  name: string;
  values: EnumValue[];
  dbName?: string | null;
...
declare type Deprecation = ReadonlyDeep2<{
  sinceVersion: string;
  reason: string;
  plannedRemovalVersion?: string;
}>;

declare type DeserializedResponse = Record<string, unknown>[];
...
export declare function dmmfToRuntimeDataModel(
  dmmfDataModel: dmmf2.Datamodel
): RuntimeDataModel;

declare type Document_2 = ReadonlyDeep2<{
  datamodel: Datamodel;
  schema: Schema;
  mappings: Mappings;
}>;
...
declare interface Engine<InteractiveTransactionPayload = unknown> {
  ...
  transaction(
    action: 'start',
    headers: transaction2.TransactionHeaders,
    options: transaction2.Options
  ): Promise<transaction2.InteractiveTransactionInfo<unknown>>;
  transaction(
    action: 'commit',
    headers: transaction2.TransactionHeaders,
    info: transaction2.InteractiveTransactionInfo<unknown>
  ): Promise<void>;
  transaction(
    action: 'rollback',
    headers: transaction2.TransactionHeaders,
...
declare interface EngineConfig {
  ...
  transactionOptions: transaction2.Options;
  /**
...
declare type EnumValue = ReadonlyDeep2<{
  name: string;
  dbName: string | null;
}>;
...
declare type ErrorCapturingFunction<T> = T extends (
  ...
  ? (...args: A) => Promise<Result4<ErrorCapturingInterface<R>>>
  : T extends (...args: infer A) => infer R
    ? (...args: A) => Result4<ErrorCapturingInterface<R>>
    : T;
...
declare type Field = ReadonlyDeep2<{
  kind: FieldKind;
  name: string;
  isRequired: boolean;
  isList: boolean;
...
declare type FieldDefault = ReadonlyDeep2<{
  name: string;
...
declare type FieldRefType = ReadonlyDeep2<{
  name: string;
...
declare function getBatchRequestPayload(
  batch: JsonQuery[],
  transaction?: TransactionOptions2<unknown>
...
declare function getExtensionContext<T>(that: T): Context2<T>;
...
export declare function getPrismaClient(config: GetPrismaClientConfig): {
  new (
     ...
     * @param typedSql
     */
    $queryRawTyped(typedSql: UnknownTypedSql): PrismaPromise2<unknown, any>;
    /**
 ...
 */
declare type HrTime2 = [number, number];

declare type Index = ReadonlyDeep2<{
  model: string;
  type: IndexType;
...
declare type IndexField = ReadonlyDeep2<{
  name: string;
  sortOrder?: SortOrder;
  length?: number;
...
declare type InputType = ReadonlyDeep2<{
  name: string;
  constraints: {
...
declare type InteractiveTransactionOptions<Payload> =
  transaction2.InteractiveTransactionInfo<Payload>;
...
declare type InternalRequestParams = {
  ...
  argsMapper?: (args?: UserArgs2) => UserArgs2;
...
declare type Mappings = ReadonlyDeep2<{
...
declare type Model = ReadonlyDeep2<{
  name: string;
  dbName: string | null;
  schema: string | null;
  fields: Field[];
...
declare type ModelMapping = ReadonlyDeep2<{
  model: string;
  plural: string;
...
declare type OtherOperationMappings = ReadonlyDeep2<{
  read: string[];
  write: string[];
}>;

declare type OutputType = ReadonlyDeep2<{
  name: string;
  fields: SchemaField[];
}>;
...
declare type PrimaryKey = ReadonlyDeep2<{
  name: string | null;
  fields: string[];
}>;
...
export declare class PrismaClientValidationError extends Error {
  name: string;
  clientVersion: string;
  constructor(message: string, { clientVersion }: Options2);
  get [Symbol.toStringTag](): string;
}
 ...
 */
declare interface PrismaPromise2<
  TResult,
...
declare type Query = ReadonlyDeep2<{
  name: string;
  args: SchemaArg[];
  output: QueryOutput;
}>;
...
declare type QueryOutput = ReadonlyDeep2<{
  name: string;
  isRequired: boolean;
  isList: boolean;
}>;
...
declare type RequestBatchOptions<InteractiveTransactionPayload> = {
  transaction?: TransactionOptions2<InteractiveTransactionPayload>;
  traceparent?: string;
  numTry?: number;
...
export declare type Result2<T, A, F extends Operation> = Result<T, A, F>;
...
declare type Result4<T> = {
  map<U>(fn: (value: T) => U): Result4<U>;
  flatMap<U>(fn: (value: T) => Result4<U>): Result4<U>;
} & (
  | {
      readonly ok: true;
      readonly value: T;
...
declare type RuntimeEnum = Omit<dmmf2.DatamodelEnum, 'name'>;

declare type RuntimeModel = Omit<dmmf2.Model, 'name'>;
...
declare type Schema = ReadonlyDeep2<{
...
declare type SchemaArg = ReadonlyDeep2<{
  name: string;
  comment?: string;
  isNullable: boolean;
  isRequired: boolean;
...
declare type SchemaEnum = ReadonlyDeep2<{
  name: string;
  values: string[];
}>;

declare type SchemaField = ReadonlyDeep2<{
  name: string;
  isNullable?: boolean;
 ...
 */
declare type TimeInput = HrTime2 | number | Date;
...
declare namespace transaction2 {
  export type {
    Options,
...
declare type TransactionOptions2<InteractiveTransactionPayload> =
  | {
      kind: 'itx';
...
declare type uniqueIndex = ReadonlyDeep2<{
  name: string;
  fields: string[];
}>;
