...
vi.mock('@ai-sdk/google', () => ({
  createGoogleGenerativeAI: vi.fn().mockImplementation((apiKey: string) => ({
    chat: vi.fn(() => ({ model: 'gemini-1.5-flash', provider: 'google', apiKey })),
    embeddings: vi.fn(() => ({ model: 'text-embedding-004', provider: 'google', apiKey })),
  })),
}));
